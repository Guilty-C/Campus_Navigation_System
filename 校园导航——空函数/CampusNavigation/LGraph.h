...
namespace Graph {
    ...
    struct EdgeNode {
        ...
        GElemSet weight; /* 权重 */
        EdgeNode(Vertex from, Vertex dest, GElemSet weight) : from(from), dest(dest), weight(weight) {}
    };
    ...
    struct HeadNode {
        ...
        explicit HeadNode(VertInfo data_) : data(std::move(data_)), adj(std::list<EdgeNode>()) {};
    };
